...
class ED5:
    ...
    def __init__(self, filename):
        ...
        mySegment = Segment(data, self)
        
        self.childs = mySegment.segments_from_data(data,self)
    ...
    def edl(self, edl_filename, filename_as_reel , gvg_format=False):
        ...
        err = {}
        for c in self.edit_cells[:]:
            if c['track'] not in 'V1 A1 A2 A3 A4'.split():
                if c['track'] not in err.keys():
                    logging.error('channel %s ignored in EDL' % c['track'])
...
class Segment:
    ...
    def segments_from_data(self, data, parent):
    ...
    def __init__(self, data, parent):
        ...
        mySegment2 = Subsegment(subsegment_data, self)
        self.childs = mySegment2.subsegments_from_data(subsegment_data, self)
        

        
class Subsegment:

    def subsegments_from_data(self, data, parent):
...
def hexdump(data, offset=0, n=None):
    ...
    while data:
        ...
        str3 = ''.join(map(lambda x: chr(x).isprintable()
                           and chr(x) or '.', line))
        print('%04x  %s %s' % (offset, str2.ljust(50), str3))
        offset += 16
