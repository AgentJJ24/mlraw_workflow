...
for folder in "$setupvar1"; do
    [ -d $folder ] && cd "$folder"
    if [[ $folder = $starissue ]]; then  # helps us not to process anything besides a numeric reel folder
...
echo "Finished with Setup Process !!!"
fi
###############################################################################################################################################################
############################################################### END SETUP PROCESS #############################################################################
###############################################################################################################################################################
...
if [[ -z $2 ]]; then
echo "ATTENTION: Specify second argument as REEL Folder(s) to offline. './#' for the specific reel folder to process all dng sequences from (# is the Reel Number Folder) "
...
echo "Process all DNGs found in $(pwd) ..."
...
for ii in *.DNG; do
    ...
    bakefile2="${filename2%.*}"
        ...
        # ADH Interpolation Debayer (-q)  # Median Filter (-m)  # Wavelet Noise Reduction (-n)  # 16-bit Linear Tiff (-4 -T)
            ...
            rm -f ./"$bakefile2".tiff
            echo -ne "        Processing Offline Proxies: '${bakefile2:0:8}'. Frame: '${bakefile2:18:7}' \r"
done
                    ...
                    if [[ $reelFolderCheck != "" ]]; then
                        ...
                        cd "$REELNAME"*
                            ...
                            if [ "$REELNAME" != "$prevREEL" ] && [ "$archiveCheck" != "" ]; then
                                ...
                                xz -d -k *.xz >> quiet.txt 2>&1
                                tar -xvkf *.tar >> quiet.txt 2>&1
                                rm -rf ./*.tar >> quiet.txt 2>&1
                            else
                            ...
                            if [ "$movCheck" != "" ]; then
                                ...
                                while [ $framecounter -lt $totalframes ]; do
                                    ...
                                    ../../ctlrender -format tiff16 -verbose -ctl ../../MOVtoTIFFrawspace.ctl ./"$REELTCNAME"_"$currFrame".tiff ./temp"$REELTCNAME"_"$currFrame".tiff
